...
export async function checkGeofences(subjectId: number, lat: number, lng: number) {
  ...
  for (const fence of rows) {
    ...
    if (prev && prev !== now) {
      ...
      await pool.query(
        `INSERT INTO events (subject_id, geofence_id, type) VALUES ($1, $2, $3)`,
        [subjectId, fence.id, type]
      );
      events.push({ geofenceId: fence.id, geofenceName: fence.name, type });
    }
